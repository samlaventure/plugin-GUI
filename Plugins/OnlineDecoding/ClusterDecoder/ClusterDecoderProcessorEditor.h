...
#ifndef CLUSTERDECODERPROCESSOREDITOR_H_INCLUDED
#define CLUSTERDECODERPROCESSOREDITOR_H_INCLUDED
...
*/
class ClusterDecoderProcessorEditor : public VisualizerEditor, public ComboBox::Listener
{
public:
    /** The class constructor, used to initialize any members. */
    ClusterDecoderProcessorEditor (GenericProcessor* parentNode, bool useDefaultParameterEditors);

    /** The class destructor, used to deallocate memory */
    ~ClusterDecoderProcessorEditor();
    ...
    */
    ...
    void setGroupsAndUnitsOptions(std::vector<uint> nUnits);
    uint getSelectedGroup();
    uint getSelectedUnit();
    double getThreshold() {return thresholdSlider->getValue();}
...
private:
    ...
    ClusterDecoderProcessorContentComponent content;

    void setFile(String file);
    void clearEditor();
    File lastFilePath;
    ScopedPointer<UtilityButton> fileButton;
    ScopedPointer<Label> fileNameLabel, thresholdLabel;
    ScopedPointer<Slider> thresholdSlider;
    ScopedPointer<ComboBox> sleepStateComboBox;


    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ClusterDecoderProcessorEditor);
};

#endif // CLUSTERDECODERPROCESSOREDITOR_H_INCLUDED
