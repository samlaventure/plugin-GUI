...
static const int EDITOR_DESIRED_WIDTH = 400;


SleepScorerProcessorEditor::SleepScorerProcessorEditor (GenericProcessor* parentNode, bool useDefaultParameterEditors = true)
    : VisualizerEditor (parentNode, useDefaultParameterEditors)
{
    ...
    content.gammaChannelEntry->addListener(this);
    content.gammaThresholdEntry->addListener(this);
    content.thetaChannelEntry->addListener(this);
    content.thetaThresholdEntry->addListener(this);
...
SleepScorerProcessorEditor::~SleepScorerProcessorEditor()
{
}


void SleepScorerProcessorEditor::editSleepScoringDisplay(const String& newSleepState)
{
...
*/
void SleepScorerProcessorEditor::buttonEvent (Button* button)
{
...
*/
void SleepScorerProcessorEditor::sliderEvent (Slider* slider)
{
}

void SleepScorerProcessorEditor::textEditorReturnKeyPressed (TextEditor& text)
{
    if (&text == content.gammaChannelEntry) {
        getProcessor()->setParameter(2, std::stoi(text.getText().toStdString()));
    } else if (&text == content.gammaThresholdEntry) {
        getProcessor()->setParameter(0, std::stod(text.getText().toStdString()));
    } else if (&text == content.thetaChannelEntry) {
        getProcessor()->setParameter(3, std::stoi(text.getText().toStdString()));
    } else if (&text == content.thetaThresholdEntry) {
        getProcessor()->setParameter(1, std::stod(text.getText().toStdString()));
    }
}


Visualizer* SleepScorerProcessorEditor::createNewCanvas()
{
    auto processor = dynamic_cast<SleepScorerProcessor*> (getProcessor());
    return new SleepScorerProcessorVisualizer (processor);
}
