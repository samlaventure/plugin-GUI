...
#ifndef SLEEPSCORERPROCESSOR_H_INCLUDED
#define SLEEPSCORERPROCESSOR_H_INCLUDED
...
class SleepScorerProcessor : public GenericProcessor
...
public:
    ...
    */
    ...
    static const char* sleepStateAsText(const SleepState sleepState);
    static const SleepState sleepStateFromText(String sleepState);

private:
    const char* sleepStateAsText();
    void computeSleepState();
    SleepState sleepState = undefinedSleepState;
    float gammaAndRatioValues[2];

    int lastSleepScoringTimestamp;
    int numOldSamplesInNewSample = 0;
    float newGammaSample = .0;
    ...
    ScopedPointer<SleepScorerFilter> gammaFilter;
    ScopedPointer<SleepScorerFilter> thetaFilter;
    ScopedPointer<SleepScorerFilter> deltaFilter;
    SleepScoringThread scoringThread;
    
    EventChannel* textChannel = 0;

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SleepScorerProcessor);
};


#endif  // SLEEPSCORERPROCESSOR_H_INCLUDED
