...
SpikeSorterCanvasWithStim::SpikeSorterCanvasWithStim(SpikeSorterWithStim* n) :
    processor(n), newSpike(false)
{
    electrode = nullptr;
    viewport = new Viewport();
    spikeDisplay = new SpikeThresholdDisplayWithStim(n,this, viewport);
    ...
    nextElectrode = new UtilityButton("Next ElectrodeWithStim", Font("Small Text", 13, Font::plain));
    nextElectrode->setRadius(3.0f);
    nextElectrode->addListener(this);
    addAndMakeVisible(nextElectrode);

    prevElectrode = new UtilityButton("Prev ElectrodeWithStim", Font("Small Text", 13, Font::plain));
...
SpikeSorterCanvasWithStim::~SpikeSorterCanvasWithStim()
{

}

void SpikeSorterCanvasWithStim::beginAnimation()
{
    std::cout << "SpikeSorterCanvasWithStim beginning animation." << std::endl;

    startCallbacks();
}

void SpikeSorterCanvasWithStim::endAnimation()
{
    std::cout << "SpikeSorterCanvasWithStim ending animation." << std::endl;

    stopCallbacks();
}

void SpikeSorterCanvasWithStim::update()
{

    std::cout << "Updating SpikeSorterCanvasWithStim" << std::endl;
    ...
    if (nPlots > 0)
    {
        ...
        SpikeHistogramPlotWithStim* sp = spikeDisplay->addSpikePlot(processor->getNumberOfChannelsForElectrode(currentElectrode), electrode->electrodeID,
...
void SpikeSorterCanvasWithStim::refreshState()
{
    // called when the component's tab becomes visible again
    resized();
}

void SpikeSorterCanvasWithStim::resized()
{
...
void SpikeSorterCanvasWithStim::paint(Graphics& g)
{
...
void SpikeSorterCanvasWithStim::refresh()
{
    // called every 10 Hz
    processSpikeEvents();
...
void SpikeSorterCanvasWithStim::processSpikeEvents()
{


    //ElectrodeWithStim* e = ((SpikeSorterWithStim*) processor)->getActiveElectrode();
...
void SpikeSorterCanvasWithStim::removeUnitOrBox()
{
    ...
    if (unitID > 0)
    {
        if (boxID >= 0)
        {
            ...
            else
            {
                ...
                std::vector<BoxUnitWithStim> boxunits = processor->getActiveElectrode()->spikeSort->getBoxUnits();
                std::vector<PCAUnitWithStim> pcaunits = processor->getActiveElectrode()->spikeSort->getPCAUnits();
                if (boxunits.size() > 0)
                {
        ...
        if (selectNewPCAUnit)
        {
            ...
            std::vector<PCAUnitWithStim> u = processor->getActiveElectrode()->spikeSort->getPCAUnits();
            if (u.size() > 0)
            {
        ...
        {
            // set new selected unit to be the last existing unit
            std::vector<BoxUnitWithStim> u = processor->getActiveElectrode()->spikeSort->getBoxUnits();
            if (u.size() > 0)
            {
...
bool SpikeSorterCanvasWithStim::keyPressed(const KeyPress& key)
{
...
void SpikeSorterCanvasWithStim::buttonClicked(Button* button)
{
    int channel = 0;
    int unitID = -1;
    int boxID = -1;
    ...
    {
        SpikeSorterEditorWithStim* ed = (SpikeSorterEditorWithStim*)processor->getEditor();
...
SpikeThresholdDisplayWithStim::SpikeThresholdDisplayWithStim(SpikeSorterWithStim* p, SpikeSorterCanvasWithStim* sdc, Viewport* v) :
...
SpikeThresholdDisplayWithStim::~SpikeThresholdDisplayWithStim()
{

}

void SpikeThresholdDisplayWithStim::clear()
{
...
void SpikeThresholdDisplayWithStim::removePlots()
{
    spikePlots.clear();

}

SpikeHistogramPlotWithStim* SpikeThresholdDisplayWithStim::addSpikePlot(int numChannels, int electrodeID, String name_)
{

    std::cout << "Adding new spike plot." << std::endl;

    SpikeHistogramPlotWithStim* spikePlot = new SpikeHistogramPlotWithStim(processor,canvas, electrodeID, 1000 + numChannels, name_);
...
void SpikeThresholdDisplayWithStim::paint(Graphics& g)
{
...
void SpikeThresholdDisplayWithStim::setPolygonMode(bool on)
{
    if (spikePlots.size() > 0)
        spikePlots[0]->setPolygonDrawingMode(on);
}

void SpikeThresholdDisplayWithStim::resized()
{
...
void SpikeThresholdDisplayWithStim::mouseDown(const MouseEvent& event)
{

}

void SpikeThresholdDisplayWithStim::plotSpike(SorterSpikeWithStimPtr spike, int electrodeNum)
{
...
SpikeHistogramPlotWithStim::SpikeHistogramPlotWithStim(SpikeSorterWithStim* prc,SpikeSorterCanvasWithStim* sdc, int electrodeID_, int p, String name_) :
...
void SpikeHistogramPlotWithStim::setSelectedUnitAndBox(int unitID, int boxID)
{
    const ScopedLock myScopedLock(mut);
    processor->getActiveElectrode()->spikeSort->setSelectedUnitAndBox(unitID, boxID);
}

void SpikeHistogramPlotWithStim::getSelectedUnitAndBox(int& unitID, int& boxID)
{
    const ScopedLock myScopedLock(mut);
...
SpikeHistogramPlotWithStim::~SpikeHistogramPlotWithStim()
{
    pAxes.clear();
    wAxes.clear();
...
void SpikeHistogramPlotWithStim::paint(Graphics& g)
{
...
void SpikeHistogramPlotWithStim::setFlipSignal(bool state)
{
...
void SpikeHistogramPlotWithStim::setPolygonDrawingMode(bool on)
{
    const ScopedLock myScopedLock(mut);
    pAxes[0]->setPolygonDrawingMode(on);
}

void SpikeHistogramPlotWithStim::updateUnitsFromProcessor()
{
    const ScopedLock myScopedLock(mut);
...
void SpikeHistogramPlotWithStim::setPCARange(float p1min, float p2min, float p1max, float p2max)
{
    const ScopedLock myScopedLock(mut);
    pAxes[0]->setPCARange(p1min, p2min, p1max, p2max);
}

void SpikeHistogramPlotWithStim::processSpikeObject(SorterSpikeWithStimPtr s)
{
    const ScopedLock myScopedLock(mut);
    if (nWaveAx > 0)
    {
...
void SpikeHistogramPlotWithStim::select()
{
    isSelected = true;
}

void SpikeHistogramPlotWithStim::deselect()
{
    isSelected = false;
}

void SpikeHistogramPlotWithStim::initAxes(std::vector<float> scales)
{
    const ScopedLock myScopedLock(mut);
    initLimits();
    ...
    for (int i = 0; i < nWaveAx; i++)
    {
        WaveformAxesWithStim* wAx = new WaveformAxesWithStim(this,processor, electrodeID, i);
    ...
    PCAProjectionAxesWithStim* pAx = new PCAProjectionAxesWithStim(processor);
...
void SpikeHistogramPlotWithStim::resized()
{
    const ScopedLock myScopedLock(mut);
...
void SpikeHistogramPlotWithStim::modifyRange(std::vector<float> values)
{
...
void SpikeHistogramPlotWithStim::modifyRange(int index,bool up)
{
    const int NUM_RANGE = 7;
    float range_array[NUM_RANGE] = {100,250,500,750,1000,1250,1500};
    String label;
...
void SpikeHistogramPlotWithStim::buttonClicked(Button* button)
{
...
void SpikeHistogramPlotWithStim::setLimitsOnAxes()
{
    const ScopedLock myScopedLock(mut);
    for (int i = 0; i < nWaveAx; i++)
...
void SpikeHistogramPlotWithStim::initLimits()
{
    for (int i = 0; i < nChannels; i++)
    {
...
void SpikeHistogramPlotWithStim::getBestDimensions(int* w, int* h)
{
    switch (plotType)
    {
        case TETRODE_PLOT:
...
void SpikeHistogramPlotWithStim::clear()
{
    const ScopedLock myScopedLock(mut);
...
void SpikeHistogramPlotWithStim::setDisplayThresholdForChannel(int i, float f)
{
    const ScopedLock myScopedLock(mut);
    if (i < wAxes.size())
...
float SpikeHistogramPlotWithStim::getDisplayThresholdForChannel(int i)
{
    const ScopedLock myScopedLock(mut);
...
GenericDrawAxesWithStim::GenericDrawAxesWithStim(int t)
...
GenericDrawAxesWithStim::~GenericDrawAxesWithStim()
{

}

bool GenericDrawAxesWithStim::updateSpikeData(SorterSpikeWithStimPtr newSpike)
{
    if (!gotFirstSpike)
    {
        gotFirstSpike = true;
...
void GenericDrawAxesWithStim::setYLims(double ymin, double ymax)
{
...
void GenericDrawAxesWithStim::getYLims(double* min, double* max)
{
    *min = ylims[0];
    *max = ylims[1];
}
void GenericDrawAxesWithStim::setXLims(double xmin, double xmax)
{
    xlims[0] = xmin;
    xlims[1] = xmax;
}
void GenericDrawAxesWithStim::getXLims(double* min, double* max)
{
    *min = xlims[0];
    *max = xlims[1];
}
...
void GenericDrawAxesWithStim::setType(int t)
{
...
int GenericDrawAxesWithStim::getType()
{
    return type;
}

int GenericDrawAxesWithStim::roundUp(int numToRound, int multiple)
{
    if (multiple == 0)
    {
        return numToRound;
...
void GenericDrawAxesWithStim::makeLabel(int val, int gain, bool convert, char* s)
{
    if (convert)
    {
...
double GenericDrawAxesWithStim::ad16ToUv(int x, int gain)
{
...
WaveformAxesWithStim::WaveformAxesWithStim(SpikeHistogramPlotWithStim* plt, SpikeSorterWithStim* p,int electrodeID_, int _channel) : GenericDrawAxesWithStim(_channel),  electrodeID(electrodeID_),
    signalFlipped(false),
    channel(_channel),
    drawGrid(true),
...
void WaveformAxesWithStim::mouseWheelMove(const MouseEvent& event, const MouseWheelDetails& wheel)
{
...
void WaveformAxesWithStim::setSignalFlip(bool state)
{
    signalFlipped = state;
    repaint();
}

void WaveformAxesWithStim::setRange(float r)
{
...
void WaveformAxesWithStim::plotSpike(SorterSpikeWithStimPtr s, Graphics& g)
{
...
void WaveformAxesWithStim::drawThresholdSlider(Graphics& g)
{
...
void WaveformAxesWithStim::drawWaveformGrid(Graphics& g)
{
...
bool WaveformAxesWithStim::updateSpikeData(SorterSpikeWithStimPtr s)
{
    if (!gotFirstSpike)
    {
        gotFirstSpike = true;
...
bool WaveformAxesWithStim::checkThreshold(SorterSpikeWithStimPtr s)
{
...
void WaveformAxesWithStim::clear()
{
...
void WaveformAxesWithStim::mouseMove(const MouseEvent& event)
{
...
int WaveformAxesWithStim::findUnitIndexByID(int ID)
{
...
void WaveformAxesWithStim::mouseDown(const juce::MouseEvent& event)
{
    ...
    //	MouseUnitOffset = ScreenToMS_uV(e.X, e.Y) - new PointDWithStim(boxOnDown.x, boxOnDown.y);
...
void WaveformAxesWithStim::mouseUp(const MouseEvent& event)
{
    if (bDragging)
    {
        bDragging = false;
...
void WaveformAxesWithStim::mouseDrag(const MouseEvent& event)
{
    bDragging = true;
    ...
    if (isOverUnit > 0)
    {
        ...
        //void WaveformAxesWithStim::isOverUnitBox(float x, float y, int &UnitID, int &BoxID, String &where)
    }
    else  if (isOverThresholdSlider)
    {
        ...
        SpikeSorterEditorWithStim* edt = (SpikeSorterEditorWithStim*) processor->getEditor();
...
void WaveformAxesWithStim::mouseExit(const MouseEvent& event)
{
    if (isOverThresholdSlider)
    {
        isOverThresholdSlider = false;
...
float WaveformAxesWithStim::getDisplayThreshold()
{
    return displayThresholdLevel;
}

void WaveformAxesWithStim::setDetectorThreshold(float t)
{
...
void WaveformAxesWithStim::isOverUnitBox(float x, float y, int& UnitID, int& BoxID, String& where)
{
    ...
    for (int k = 0; k < units.size(); k++)
    {
        ...
        {
            BoxWithStim B = units[k].lstBoxes[boxiter];
...
void WaveformAxesWithStim::drawBoxes(Graphics& g)
{
...
void WaveformAxesWithStim::updateUnits(std::vector<BoxUnitWithStim> _units)
{
    units = _units;
}

void WaveformAxesWithStim::paint(Graphics& g)
{
...
PCAProjectionAxesWithStim::PCAProjectionAxesWithStim(SpikeSorterWithStim* p) : GenericDrawAxesWithStim(0), processor(p), imageDim(500),
...
void PCAProjectionAxesWithStim::resized()
{
...
void PCAProjectionAxesWithStim::setPolygonDrawingMode(bool on)
{
    if (on)
    {
...
void PCAProjectionAxesWithStim::updateUnits(std::vector<PCAUnitWithStim> _units)
{
    units = _units;
}

void PCAProjectionAxesWithStim::drawUnit(Graphics& g, PCAUnitWithStim unit)
{
    float w = getWidth();
    float h = getHeight();
...
void PCAProjectionAxesWithStim::paint(Graphics& g)
{
    ...
    if (inPolygonDrawingMode)
    {
        ...
        if (drawnPolygon.size() > 0)
        {
            ...
            for (std::list<PointDWithStim>::iterator it = drawnPolygon.begin(); it != drawnPolygon.end(); it++)
            {
                if (first)
                {
                    first = false;
...
void PCAProjectionAxesWithStim::drawProjectedSpike(SorterSpikeWithStimPtr s)
{
...
void PCAProjectionAxesWithStim::redraw(bool subsample)
{
    Graphics g(projectionImage);
...
void PCAProjectionAxesWithStim::setPCARange(float p1min, float p2min, float p1max, float p2max)
{
...
bool PCAProjectionAxesWithStim::updateSpikeData(SorterSpikeWithStimPtr s)
{
...
void PCAProjectionAxesWithStim::clear()
{
    projectionImage.clear(juce::Rectangle<int>(0, 0, projectionImage.getWidth(), projectionImage.getHeight()),
                          Colours::black);
...
void PCAProjectionAxesWithStim::mouseDrag(const juce::MouseEvent& event)
{
    ...
    else
    {
        ...
        {
            drawnPolygon.push_back(PointDWithStim(event.x,event.y));
            // draw polygon
            prevx = event.x;
            prevy = event.y;
...
void PCAProjectionAxesWithStim::mouseUp(const juce::MouseEvent& event)
{
    repaint();
    //redraw(false);
    setMouseCursor(MouseCursor::NormalCursor);
    ...
    if (inPolygonDrawingMode)
    {
        ...
        for (std::list<PointDWithStim>::iterator it = drawnPolygon.begin(); it != drawnPolygon.end(); it++,k++)
        {
...
void PCAProjectionAxesWithStim::mouseMove(const juce::MouseEvent& event)
{
    isOverUnit = -1;
    float w = getWidth();
    float h = getHeight();
    ...
    for (int k=0; k<units.size(); k++)
    {
        ...
        if (units[k].isPointInsidePolygon(PointDWithStim(x1,y1)))
        {
...
void PCAProjectionAxesWithStim::mouseDown(const juce::MouseEvent& event)
{
    prevx = event.x;
    prevy = event.y;
    if (event.mods.isRightButtonDown())
    ...
    if (inPolygonDrawingMode)
    {
        drawnUnit = PCAUnitWithStim(processor->getActiveElectrode()->spikeSort->generateUnitID(),processor->getActiveElectrode()->spikeSort->generateLocalID());
        drawnPolygon.push_back(PointDWithStim(event.x,event.y));
    }
    else
    {
...
bool PCAProjectionAxesWithStim::keyPressed(const KeyPress& key)
{
    KeyPress e = KeyPress::createFromDescription("escape");
...
void PCAProjectionAxesWithStim::rangeDown()
{
    float range0 = pcaMax[0]-pcaMin[0];
    float range1 = pcaMax[1]-pcaMin[1];
...
void PCAProjectionAxesWithStim::rangeUp()
{
    float range0 = pcaMax[0]-pcaMin[0];
    float range1 = pcaMax[1]-pcaMin[1];
    pcaMin[0] = pcaMin[0] + 0.1 * range0;
...
void PCAProjectionAxesWithStim::buttonClicked(Button* button)
{
...
void PCAProjectionAxesWithStim::mouseWheelMove(const MouseEvent& event, const MouseWheelDetails& wheel)
{
    if (wheel.deltaY > 0)
        rangeDown();
    else
