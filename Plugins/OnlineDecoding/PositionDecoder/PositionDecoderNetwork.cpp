	...
	channels(inChannels)
{
	...
	for (uint group=0 ; group<channels.size() ; ++group) {
		...
		dataTensors.push_back(tensorflow::Tensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({100, (int) channels.at(group).size(), 32})));
	}
	clearOutput();
		...
		for (uint s=0; s<100; s++) {
			for (uint c=0; c<channels.at(g).size(); c++) {
				for (uint spl=0; spl<32; spl++) {
					dataTensors.at(g).tensor<float,3>()(s, c, spl) = 0;
				}
			}
		}
	}
...
void PositionDecoderNetwork::addSpike(std::vector<std::vector<float>>& data, int inGroup)
{
	...
	// 	nSpikes.at(group) += 1;
	// }
	if (nSpikes >= 99) return;
	for (uint c=0; c<channels.at(inGroup).size(); c++) {
		for (uint spl=0; spl<32; spl++) {
			dataTensors.at(inGroup).tensor<float, 3>()(nSpikes, c, spl) = data.at(c).at(spl);
		}
	}
	nSpikes += 1;
}
...
OnlineDecoding::DecodingResults* PositionDecoderNetwork::inferPosition()
{
	...
	std::cout << "nSpikes: " << nSpikes << " ";
	for (uint g=0; g<channels.size(); g++) {
		sessionInput.at(g).second = dataTensors.at(g).Slice(0, nSpikes);
	}
